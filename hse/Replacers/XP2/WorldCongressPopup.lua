...
function PopulateLeaderStack()
	...
	for _, pPlayer in ipairs(aPlayers) do
		...
		if hasMetPlayer or (GameConfiguration.IsAnyMultiplayer() and pPlayerConfig:IsHuman()) then
			...
			if hasMetPlayer then
				...
				if HasAccessLevel(playerID, "economy") then
					...
					instance.FavorLabel:SetText(Locale.Lookup("LOC_WORLD_CONGRESS_FAVOR", hseUnknown));
					instance.FavorLabel:SetToolTipString(Locale.Lookup(playerID == localPlayerID and "LOC_WORLD_CONGRESS_TT_PLAYER_FAVOR" or "LOC_WORLD_CONGRESS_TT_LEADER_FAVOR", hseUnknown));
				end
				instance.FavorContainer:SetHide(false);


				if not hideGrievances or not isCUIActive then
				...
				if hideGrievances and not isCUIActive then
					instance.GrievanceContainer:SetHide(true);
				end
			end
			
			if hideGrievances and isCUIActive then
		...
		if (isBLIActive or isCUIActive) and isEDRActive then
			...
			UpdateLeaderIcon(uiLeaderIcon, nil, playerID, nil, true)
		end

		if isCUIActive and not isBLIActive and not isEDRActive then
