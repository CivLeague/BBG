...
function ComputeDominanceText(numDominantCities, dominantString, numFollowers, followerString)	
	if(numDominantCities == 1) then
		text = Locale.ToUpper(Locale.Lookup("LOC_UI_RELIGION_RELIGION_DOMINANCE", (dominantString)));
	else
		text = Locale.ToUpper(Locale.Lookup("LOC_UI_RELIGION_RELIGION_DOMINANCE_PLURAL", (dominantString)));
	end
	
	if numFollowers ~= nil then
		text = text .. string.format(" [ICON_Citizen]%s", (followerString));
	end
	
	return text
end

function ComputeDominanceTextReligiousDetails(religiousDetails, withFollowers)
	if withFollowers == true then
		return ComputeDominanceText(religiousDetails.TotalMinNumConvertedCities, religiousDetails.ConvertedRangeString, religiousDetails.NumTotalFollowersVisible, religiousDetails.FollowerString)

	else
		return ComputeDominanceText(religiousDetails.TotalMinNumConvertedCities, religiousDetails.ConvertedRangeStringMajor)
	end
end
...
function SelectReligionBeliefs()
	...
	if(m_NumBeliefsEquipped == 0) then
		...
		local numDominantCitiesHidden:number = 0
		...
		for _, player in ipairs(majorPlayers) do
			...
			for _, city in playerCities:Members() do
				...
				if(cityReligion:GetMajorityReligion() == m_PlayerReligionType) then
					...
					if not pLocalPlayerVis:IsRevealed(city:GetX(), city:GetY()) then
						numDominantCitiesHidden = numDominantCitiesHidden + 1
					end
				end
			end
		end
		...
		Controls.AddBeliefsReligionDominance:SetText(ComputeDominanceTextReligiousDetails(GetReligiousDetails(m_SelectedReligion.ID)))
	end
...
function ViewReligion(religionType:number)
	...
	for _, player in ipairs(majorPlayers) do
		...
		for _, city in playerCities:Members() do
			...
			-- Adriaman: added condition on revealed plot
			if not IsMaskingActive() or pLocalPlayerVis:IsRevealed(city:GetX(), city:GetY()) or Game.GetLocalObserver() == PlayerTypes.OBSERVER then
	...
	Controls.ViewReligionDominance:SetText(ComputeDominanceTextReligiousDetails(GetReligiousDetails(religionType), true))
...
function ViewAllReligions()
	...
	for _, player in ipairs(majorPlayers) do
		...
		for _, city in playerCities:Members() do
			...
			if(dominantCities[majorityReligion] == nil) then
				...
				dominantCitiesHidden[majorityReligion] = 1
			else
				dominantCities[majorityReligion] = dominantCities[majorityReligion] + 1;
				
				if not pLocalPlayerVis:IsRevealed(city:GetX(), city:GetY()) then
					dominantCitiesHidden[majorityReligion] = dominantCitiesHidden[majorityReligion] + 1;
				end
			end
		end
	end
	...
	for _, religionInfo in ipairs(allReligions) do
		...
		if(religionData.Pantheon == false and m_pGameReligion:HasBeenFounded(religionInfo.Religion)) then
			...
			local numDominantCitiesHidden = dominantCitiesHidden[religionInfo.Religion];
			
			if numDominantCities == nil then numDominantCities = 0; numDominantCitiesHidden=0; end
			--[[
			if(numDominantCities == 1) then
			...
			religionInst.ReligionDominance:SetText(ComputeDominanceTextReligiousDetails(GetReligiousDetails(religionInfo.Religion)))
