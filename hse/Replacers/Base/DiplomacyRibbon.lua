...
function AddLeader(iconName : string, playerID : number, kProps: table)
	...
	if isCUIActive and not isEDRActive and not isBLIActive then
...
function UpdateStatValues( playerID:number, uiLeader:table )
	...
	if uiLeader.Score:IsVisible() then 
		...
		if HasAccessLevel(playerID, "score") then
			uiLeader.Score:SetText("[ICON_Capital]"..tostring(score));
		else
			uiLeader.Score:SetText("[ICON_Capital] " .. hseUnknown);
		end
	end
	...
	if uiLeader.Military:IsVisible() then	
		...
		if HasAccessLevel(playerID, "military") then
			uiLeader.Military:SetText( "[ICON_Strength]"..tostring(military));
		else
			uiLeader.Military:SetText("[ICON_Strength] " .. hseUnknown);
		end
	end
	...
	if uiLeader.Gold:IsVisible() then 		
		...
		if HasAccessLevel(playerID, "economy") then
			...
			uiLeader.Gold:SetText("[ICON_Gold] " .. hseUnknown);
		end
	end
	...
	if uiLeader.Faith:IsVisible() then
		...
		if HasAccessLevel(playerID, "religion") then
			...
			uiLeader.Faith:SetText("[ICON_Faith] " .. hseUnknown);
		end
	end
...
if not isRFActive and not isGatheringStormActive then
	if isBLIActive then
